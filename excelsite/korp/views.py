...
def page(request: HttpRequest, pagetitle='неопр', menuitem=0) -> HttpResponse:
    ...
    context = {'metatitle': metatitle, 'metadescription': metadescription, 'title': title, 'content': content,
               'page': page, 'menuitem': menuitem, 'deploy': deploy, "partners":partners}
...
def formsent(request):
    #from utils.form_client import sendform
    from utils.form_client_base import sendform

    form=sendform(request)
    return render(request, "form/formsent.html", {'form': form})
